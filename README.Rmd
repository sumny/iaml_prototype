...
spam = map_dtr(1:10, function(r) {
instance$archive$clear()
tuner = tnr("iaml_ea", mu = 200, lambda = 20)  # see ?TunerIAMLEA
...
nadir = apply(rbind(ea_data[, instance$archive$cols_y, with = FALSE], rs_data[, instance$archive$cols_y, with = FALSE]), MARGIN = 2L, FUN = function(x) max(x) + 1)
ea_hv = emoa::dominated_hypervolume(t(ea[, instance$archive$cols_y, with = FALSE]), ref = nadir)
rs_hv = emoa::dominated_hypervolume(t(rs[, instance$archive$cols_y, with = FALSE]), ref = nadir)
