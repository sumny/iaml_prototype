...
IAMLPointNEWX = R6Class("IAMLPoint",
  public = list(
    initialize = function(task, n_selected = NULL, scores = NULL, interaction_detector = NULL, unconstrained_weight_table = NULL, n_interactions_prob = NULL, unconstrained = FALSE, use_detectors = TRUE) {
      ...
      self$use_detectors = assert_flag(use_detectors)
      ...
      if (unconstrained) {
        ...
        selected_features = if (self$use_detectors) {
          sort(sample(scores$feature, size = n_selected, replace = FALSE, prob = scores$score))
        } else {
          sort(sample(task$feature_names, size = n_selected, replace = FALSE))
        }
        ...
        if (n_selected == 1L) {
          ...
          belonging = if (self$use_detectors) {
            interaction_detector$get_eqcs_from_top_k(k = k, features = selected_features)
          } else {
            setNames(sample(k, size = n_selected, replace = TRUE), nm = selected_features)
          }
        ...
        if (self$use_detectors) {
          ...
          monotone_eqcs = map_int(seq_len(length(self$groups$eqcs) - 1L), function(group) {
            sample(c(-1L, 0L, 1L), size = 1L, prob = c(0.1, 0.8, 0.1))
          })
        }
    ...
    mutate = function() {
      ...
      tryCatch(
      {
        ...
        if (sum(change_monotonicity) > 0) {
          new_monotonicity = if (self$use_detectors) {
            sample(c(0L, 1L), size = sum(change_monotonicity), replace = TRUE)
          } else {
            sample(c(-1L, 0L, 1L), size = sum(change_monotonicity), replace = TRUE)
          }
